...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomTunerserviceHdle;                    /**< TunerService handle */
  uint16_t  CustomCharcastHdle;                  /**< charCast handle */
...
#define COPY_TUNERSERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_CHARCAST_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for TunerService +
   *                                2 for charCast +
   *                                1 for charCast configuration descriptor +
   *                              = 4
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomTunerserviceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: TunerService, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: TunerService \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomTunerserviceHdle,
                          ...
                          &(CustomContext.CustomCharcastHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CHARCAST, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CHARCAST \n\r");
  }
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARCAST:
      ret = aci_gatt_update_char_value(CustomContext.CustomTunerserviceHdle,
                                       CustomContext.CustomCharcastHdle,
                                       0, /* charValOffset */
                                       SizeCharcast, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CHARCAST command, result : 0x%x \n\r", ret);
      }
      else
      {
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARCAST:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomTunerserviceHdle, CustomContext.CustomCharcastHdle, SizeCharcast, pPayload);
